...
export function FlashCard({ card, onClick, onEdit, onDelete, isDragging, canEdit, canDelete, heightClass = 'h-28', textClass = 'text-sm', clampClass = 'line-clamp-3', tabIndex, onFocus, onKeyDown, cardRef }: FlashCardProps) {
  return (
    <div className={`relative${isDragging ? ' opacity-40' : ''}`}>
      <button
        ...
        aria-expanded={false}
        className={`w-full ${heightClass} flex flex-col items-center justify-center p-3
      ...
      >
        <span className={`${textClass} font-semibold text-surface-700 dark:text-surface-100 text-balance leading-snug ${clampClass}`}>
          {card.title}
        </span>
        {card.category && (
